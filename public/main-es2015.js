...
let AppComponent = class AppComponent {
    constructor(gameService) {
        ...
        this.zz = { DataVersion: 10, Language: 'Zh-Hant', Version: '1.5.8', Country: 'TW', GameStart: 'True', DUID: 'a720d66d-3fb2-4449-85d1-b67bbe1146c4', LastAccessTime: 44043.5329359259 };
        this.updateOption = {
            updateTime: 25,
            disable: true,
            status: false
    ...
    ngOnInit() {
        ...
        this.gameService.start$.subscribe(result => {
            if (result) {
                this.update();
                this.gameService.start$.next(false);
            }
        });
        // this.getBasicInfo();
        this.gameInfo$.subscribe(result => {
            if (result) {
                console.log(result.Heroes);
                this.heroList$ = this.defaultData(result.Heroes);
            }
        });
        setInterval(() => {
            this.currentDate = new Date(new Date().getTime() + new Date().getTimezoneOffset() * 60000);
    ...
    updateGame(data) {
        const newGameInfo = lodash__WEBPACK_IMPORTED_MODULE_4__["assign"](this.gameInfo$.value, data);
        this.gameInfo$.next(newGameInfo);
    }
    keyByArray(data) {
    ...
    defaultData(data) {
        const dataList = Object.keys(data).map((v, index) => {
    ...
    update() {
        ...
        this.timeout = setInterval(() => {
            second += 1;
            this.timeoutByGameUpdateMessage = `${this.updateOption.updateTime - second} s`;
            if (this.updateOption.updateTime === second) {
                this.updateFunc();
                second = 0;
            }
            console.log(this.updateOption.updateTime - second);
        }, 1000);
    }
    updateTime(time) {
...
let GameService = class GameService {
    constructor(http) {
        ...
        //apiUrl = 'http://localhost:3000/tap-game';
        ...
        // tslint:disable-next-line: max-line-length
        this.playGameData = 'H4sIAAAAAAAAEz3NsQ6CMBSF4Xe5MyXccinVzUiiJm6iJm63UJGBkrRlMMZ3tyyO58tJ/g80HPlmfRhnB1ssMjizGxYeLGzh8RJHdhEy+D8A8yrXSfbz4qJ/J2nvaR54spfIPq7gF5uouZ6atLiWRa9UL8qnkYKINkJXPQqjamMsIqmOYM2GuOs6G0I7TilOVFCdV1KXUiOW9P0BdghRpKsAAAA=';
    }
    getBasicInfo() {
        this.http.get(`${this.apiUrl}/basicInfo`).subscribe((result) => {
            this.basicInfo$.next(result.Contents);
        });
    }
    getGameInfo() {
        this.http.get(this.updateUrl).subscribe(result => {
            this.gameInfo$.next(result);
        });
    }
    getUserInfo() {
        this.http.get(this.userUrl).subscribe((result) => {
            this.userInfo$.next(result.Contents);
        });
    }
    playGame() {
        const data = {
    ...
    update(data) {
        this.http.post(`${this.apiUrl}/update`, data).subscribe(result => {
            console.log(result);
        }, err => {
            this.getBasicInfo();
            setTimeout(() => {
    ...
    prestige() {
        this.http.get(this.prestigeUrl).subscribe((result) => {
            const req = {
                token: this.basicInfo$.getValue().Token,
                data: lodash__WEBPACK_IMPORTED_MODULE_4__["assign"](result, { ClearStage: this.gameInfo$.value.ClearStage })
            };
            this.http.post(`${this.apiUrl}/Prestige`, req).subscribe(prestigeResult => {
                if (prestigeResult) {
